...
- **Nested Mirroring Issue & Resolution**:
  - *Symptom*: When attaching the subassembly to the Right side, it rendered as a Left-side retaining wall, and toggling the properties panel did nothing.
  - *Cause*: The mathematical logic in `WallGeometryGenerator.cs` was fundamentally inverted. Generating `Side.Right` produced negative-X coordinates (Left-facing geometry), while `Side.Left` produced positive-X coordinates (Right-facing geometry). Because raw .NET subassemblies do not automatically mirror geometry based on side placement, passing the inverted logic caused backwards orientations. Additionally, a previous attempt to fix this hardcoded the generation to `Side.Right`, which broke the properties panel toggle functionality.
  - *Solution*: Corrected the sign mapping in `WallGeometryGenerator.cs` so `Side.Left = 1.0` (negative X axis) and `Side.Right = -1.0` (positive X axis). Removed the temporary `Side.Right` hardcoding in `RoadEdgeRetainingWall.cs` so it dynamically evaluates the properties panel toggle again.
- **Build & Verification**:
